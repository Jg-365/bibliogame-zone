...
test.describe('ReadQuest Authentication', () => {
  test('should display authentication page for non-logged users', async ({ page }) => {
    await page.goto('/');
    
    // Should redirect to auth page or show auth form
    await expect(page.locator('h1')).toContainText(['Entrar', 'Login', 'Sign In', 'Bem-vindo']);
  ...
  test('should show validation errors for invalid login', async ({ page }) => {
    ...
    await expect(page.locator('text=/erro|error|inválido|invalid/i')).toBeVisible({ timeout: 5000 });
  });
});
...
test.describe('ReadQuest Dashboard', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const authCheck = await page.locator('h1').textContent();
  ...
  test('should display user dashboard with main sections', async ({ page }) => {
    // Check if dashboard loads
    await expect(page.locator('h1')).toContainText('Olá');
    
    // Check main stats cards
    await expect(page.locator('text=Livros Concluídos')).toBeVisible();
    await expect(page.locator('text=Páginas Lidas')).toBeVisible();
    await expect(page.locator('text=Pontos de Experiência')).toBeVisible();
  ...
  test('should navigate between main sections', async ({ page }) => {
    ...
    const sections = ['Visão Geral', 'Biblioteca', 'Sessões', 'Conquistas'];
  ...
  test('should display streak information correctly', async ({ page }) => {
    ...
    if (await streakElement.isVisible()) {
      ...
      await expect(streakElement).toContainText(/\d+ dias?/);
      
      // Should have flame icon or similar indicator
      await expect(streakElement.locator('svg')).toBeVisible();
    }
  });
});
...
test.describe('ReadQuest Book Management', () => {
  ...
  test('should display book search functionality', async ({ page }) => {
    await page.click('text=Biblioteca');
    await page.click('text=Adicionar Livro');
    
    // Search for a book
    const searchInput = page.locator('input[placeholder*="livro" i], input[placeholder*="search" i], input[placeholder*="buscar" i]').first();
    ...
    if (await searchInput.isVisible()) {
      ...
      await expect(page.locator('text=/resultados?|loading|carregando/i')).toBeVisible({ timeout: 10000 });
    }
  });
});
...
test.describe('ReadQuest Navigation System', () => {
  ...
  test('should have responsive navigation', async ({ page }) => {
    ...
    const desktopNav = page.locator('[role="tablist"], .lg\\:block').first();
    ...
    const mobileNav = page.locator('select, .block.lg\\:hidden, [class*="bottom"]').first();
  ...
  test('should navigate to all main pages', async ({ page }) => {
    ...
    for (const pageInfo of pages) {
      ...
      await expect(page.locator('text=/404|not found|erro|error/i')).not.toBeVisible();
...
test.describe('ReadQuest Social Features', () => {
  ...
  test('should display social page', async ({ page }) => {
    ...
    await expect(page.locator('h1, h2')).toContainText(/Social|Feed|Atividade/i);
    
    // Should have post creation area or feed
    const socialElements = page.locator('text=/postar|post|compartilhar|feed/i');
  ...
  test('should display ranking page', async ({ page }) => {
    ...
    await expect(page.locator('h1, h2')).toContainText(/Ranking|Leaderboard|Classificação/i);
    
    // Should have ranking list or stats
    const rankingElements = page.locator('text=/posição|rank|#1|#2|#3/i');
...
test.describe('ReadQuest Responsive Design', () => {
  ...
  viewports.forEach(({ name, width, height }) => {
    test(`should be responsive on ${name}`, async ({ page }) => {
      ...
      const navElements = page.locator('[role="navigation"], nav, [class*="nav"]');

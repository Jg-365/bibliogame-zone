  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Activity[]> => {
      try {
        ...
        // Load only today's posts (start of local day)
        ...
        // Buscar perfis e livros em paralelo para reduzir tempo de espera
        const bookIds = posts
          .filter((post) => post.book_id)
          .map((post) => post.book_id!);
        ...
        const booksPromise = bookIds.length
          ? supabase
              .from("books")
        ...
        // Criar hashtables (Map) para consultas O(1) ao combinar dados
        const profileMap = new Map(
          (profiles || []).map((p: any) => [p.user_id, p])
        );
        const booksMap = new Map(
          (books || []).map((b: any) => [b.id, b])
        );

        // Combinar dados usando Map#get para O(1) lookup
        return posts.map((post) => {
          const profile = profileMap.get(post.user_id);
          const book = post.book_id
            ? booksMap.get(post.book_id)
            : undefined;
...
export const useEnhancedAchievements = () => {
  ...
  const enrichedAchievements: EnhancedAchievement[] =
...
export const useEnhancedLeaderboard = (
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<LeaderboardUser[]> => {
      ...
      const rankedData: LeaderboardUser[] = (
        data || []
      ).map((user, index) => ({
...
// Provide a hashtable (Map) view over the leaderboard for O(1) lookups by user_id
export const useLeaderboardHash = (
  type: "points" | "books" | "pages" | "streak" = "points",
  options?: { enabled?: boolean }
) => {
  const { data: list = [] } = useEnhancedLeaderboard(
    type,
    options
  );
  return React.useMemo(
    () =>
      new Map((list || []).map((u: any) => [u.user_id, u])),
    [list]
  );
};
...
export const usePublicActivityFeed = () => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: completedBooks, error: booksError } =
        await supabase
          .from("books")
          .select(
            "id, user_id, title, author, cover_url, updated_at, rating"
          )
          .eq("status", "lido")
          .order("updated_at", { ascending: false })
          .limit(20);
      ...
      const userIds = [
        ...new Set(
          completedBooks?.map((book) => book.user_id) || []
        ),
      ];
      ...
      achievements?.forEach((achievement) => {
        ...
        const achievementData = achievementMap.get(

...
export const ProfileGenreMetrics = ({ items }: ProfileGenreMetricsProps) => {
    ...
    <Card className="border-border/70">
      ...
      <CardContent className="grid gap-3 lg:grid-cols-2">
        {items.slice(0, 6).map((item) => (
          <div key={item.genre} className="rounded-xl border border-border/70 bg-card/80 p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <Badge variant="secondary">{Math.round(item.completionRate * 100)}% concluídos</Badge>
            </div>

...
export const Leaderboard = () => {
  ...
  const visibleUsers =
    leaderboard?.slice(0, displayCount) || [];
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            O ranking ainda não tem dados suficientes.
          </CardDescription>
        </CardHeader>
        <CardContent>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Top {displayCount} de {leaderboard.length}{" "}
          leitores mais ativos da comunidade
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-3">
          {visibleUsers.map((user, index) => (
            <div
              key={user.userId}
              className={`flex items-center gap-3 p-3 rounded-lg border transition-all hover:shadow-sm ${getRankColor(
                user.rank
              )}`}
            >
              {/* Rank */}
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
          ...
          <div className="text-xs text-blue-700 space-y-1">
            <p>
              • Complete livros para ganhar pontos (10
              pontos por livro)
            </p>
            <p>
              • Mantenha uma sequência de leitura diária
            </p>
            <p>
              • Ganhe pontos extras por páginas lidas (1
              ponto a cada 50 páginas)
            </p>
          </div>
        </div>
      </CardContent>

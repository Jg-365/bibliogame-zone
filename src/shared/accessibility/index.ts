...
/**
 * Hook for managing skip links for screen readers
 */
export const useSkipLinks = () => {
  const skipLinksRef = React.useRef<HTMLDivElement>(null);
  const [skipLinks, setSkipLinks] = React.useState<
    Array<{
      id: string;
      label: string;
      target: string;
  ...
  const addSkipLink = React.useCallback((id: string, label: string, target: string) => {
    setSkipLinks(prev => {
  ...
  const SkipLinksComponent = React.useCallback(
    () => (
      <div ref={skipLinksRef} className="sr-only focus-within:not-sr-only">
        {skipLinks.map(link => (
          <a
            key={link.id}
            href={`#${link.target}`}
            className="absolute top-0 left-0 z-50 p-2 bg-primary text-primary-foreground focus:relative focus:z-auto"
            onFocus={e => {
